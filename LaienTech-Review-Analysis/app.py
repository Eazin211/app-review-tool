...
st.set_page_config(
    page_title="LaienTech - App Review Analysis",
    page_icon="🔍",
    layout="wide",
...
def render_sidebar():
    with st.sidebar:
        ...
        goals_input = st.text_area(
            "Analysis objectives or focus areas",
            value="subscription conversion, workout availability, low-rated reviews, user experience",
            height=100,
        ...
        if api_key:
            ...
            st.warning("⚠️ No LLM API key — using statistical baseline only")

        st.markdown("---")

        max_pages = st.slider("Max RSS pages", 1, 10, 5, help="Number of pages to fetch from RSS (max 10, ~50 reviews/page)")
...
def run_analysis_pipeline(max_pages: int):
    ...
    try:
        ...
        if st.session_state.get('app_info', {}).get('is_sample'):
            ...
            source = 'rss'
            log(f"Fetched {len(reviews)} reviews from RSS feed")
            ...
            st.error("No data source configured. Please provide a URL, import data, or use sample.")
            st.session_state.running = False
            return
        ...
        log(f"Cleaned: {stats['input_count']} → {stats['output_count']} reviews")

        update_progress(4, "Analyzing reviews with AI-driven theme discovery...")
        ...
        log(f"Validation {'PASSED' if validation.get('valid') else 'ISSUES FOUND'}: {validation.get('stats', {})}")

        progress_placeholder.success("✅ Analysis pipeline completed!")
...
def render_main_content():
    ...
    st.markdown('<div class="sub-header">AI-Powered App Store Review Analysis → PRD → Test Generation Pipeline</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div style="text-align: center; padding: 40px 20px;">
            ...
            <p style="color: #666; font-size: 16px;">
                ...
                1️⃣ Collect App Store reviews via RSS feed<br>
...
def render_pipeline_status():
    ...
    stages = [
        "Collect Data", "Clean Data", "AI Analysis",
        "Generate PRD", "Generate Tests", "Validate"
    ]
...
def render_cleaned_data_tab():
    ...
    if stats:
        ...
        with col2:
            st.markdown(f'<div class="metric-card"><div style="color:#666;font-size:12px">After Cleaning</div><div style="font-size:24px;font-weight:700">{stats.get("output_count", 0)}</div></div>', unsafe_allow_html=True)
        with col3:
...
def render_analysis_tab():
    ...
    if not analysis:
        st.info("No analysis results yet. Run the pipeline first.")
        return
    ...
    with col3:
        model_status = "🤖 LLM" if analysis.get('model_used') else "📊 Statistical"
    ...
    st.markdown(f"### 🏷️ Identified Themes ({len(themes)})")
    for theme in themes:
    ...
    for finding in findings:
        ...
        with st.expander(f"{'🔴' if severity == 'high' else '🟡' if severity == 'medium' else '🟢'} {finding.get('label', 'Unknown')} | [{severity.upper()}] | {len(finding.get('source_review_ids', []))} reviews | Confidence: {confidence:.0%}"):
            st.markdown(f"**Type:** `{finding.get('type', '').replace('_', ' ').title()}`")
...
def render_prd_tab():
    ...
    if not prd:
        st.info("No PRD generated yet. Run the pipeline first.")
        return

    st.markdown("### 📋 Product Requirements Document")

    st.markdown(f"**Title:** {prd.get('title', '')}")
    st.markdown(f"**Generated:** {prd.get('generated_at', '')}")
    st.markdown(f"**{prd.get('executive_summary', '')}**")

    st.markdown("---")
    versions = prd.get('versions', [])
    st.markdown(f"### 🗺️ Release Plan ({len(versions)} phases)")
    for ver in versions:
    ...
    st.markdown(f"### 📝 Requirements ({len(requirements)})")
...
def render_testcases_tab():
    ...
    if not test_results:
        st.info("No test cases generated yet. Run the pipeline first.")
        return
    ...
    st.markdown(f"Showing {len(filtered)} of {len(test_cases)} test cases")
    ...
    for tc in filtered:
        ...
        with st.expander(f"{p_icon} {tc.get('id', '')}: {tc.get('title', '')[:80]}"):
            ...
            with col1:
                ...
                st.markdown(f"**Automation:** {'✅' if tc.get('automation_feasible') else '❌'}")
            with col2:
...
def render_validation_tab():
    ...
    if not validation:
        st.info("No validation results yet. Run the pipeline first.")
        return
